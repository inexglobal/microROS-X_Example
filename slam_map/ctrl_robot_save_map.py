...
class Yahboom_Keyboard(Node):
    ...
    def save_map(self):
        """ ฟังก์ชันสำหรับรันสคริปต์บันทึกแผนที่ """
        try:
            print("\n[System] Saving map via save_map.sh...")
            # รันสคริปต์ shell (ตรวจสอบให้แน่ใจว่าไฟล์อยู่ในโฟลเดอร์เดียวกันหรือระบุ Path เต็ม)
            subprocess.Popen(["sh", "./save_map.sh"]) 
        except Exception as e:
            print(f"\n[Error] Could not run save_map.sh: {e}")
